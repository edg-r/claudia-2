# Quantitative Methods 3 (QM3) Midterm Lecture Reference v1.3.0

This reference sheet is built from the QM3 lecture slides, not the textbook-centered methods guide. The v1.3.0 update preserves v1.2.1's exam-facing structure, formula wrapping, first-use acronym format, and "Explain Like I'm 5" conclusions, then adds the imagegen lecture maps and sharpens the causal-identification language from Edgar's learning thread.
...
Each lecture's imagegen source map appears once at the first numbered topic where that lecture's concepts are used. The maps are visual memory aids; the causal interpretation still comes from the assumptions written in the text.
...
**Visual aid:** The Lecture 1 map shows the core counterfactual problem: for the same unit, one potential outcome is observed and the other is missing.

[[IMAGE:assets/qm3_midterm_v1.3.0/l1_counterfactual_identification.png]]
...
**Visual aid:** The Lecture 2 map keeps the three objects separate: ATE is the population-average causal effect, ATT is the average causal effect given treatment status `D = 1`, and Difference in Means is the raw observed gap that becomes causal only with assumptions such as random assignment.

[[IMAGE:assets/qm3_midterm_v1.3.0/l2_ate_att_difference_in_means.png]]
...
**Visual aid:** The Lecture 3 map ties randomization, OLS, Conditional Independence Assumption (CIA), and common support together as different ways to make a comparison group believable.

[[IMAGE:assets/qm3_midterm_v1.3.0/l3_randomization_ols_common_support.png]]
...
**Visual aid:** See the Lecture 3 map above for the link between the OLS comparison, Conditional Independence Assumption (CIA), and common support. The line summarizes the comparison; the causal interpretation still depends on the error term being unrelated to the regressor after controls.
...
**Visual aid:** The Lecture 4 map makes the sign logic mechanical and places OVB beside simultaneity and measurement error. Bias requires one path from the omitted variable to the treatment/regressor and another path from the omitted variable to the outcome.

[[IMAGE:assets/qm3_midterm_v1.3.0/l4_ovb_simultaneity_measurement_error.png]]
...
**Visual aid:** The Lecture 5 map marks the bad-control danger and then connects the IV pipeline: if treatment causes the control, holding it fixed changes the question from total effect to a direct-effect comparison.

[[IMAGE:assets/qm3_midterm_v1.3.0/l5_bad_controls_iv_pipeline.png]]
...
**Visual aid:** See the Lecture 5 map above for the exam sequence: prove the instrument moves treatment, show assignment moves the outcome, then divide reduced form by first stage.
...
**Visual aid:** The Lecture 6 map keeps the compliance and LATE objects separate from ATE, ATT, ITT, and TOT. The LATE interpretation belongs to compliers, because they are the group whose treatment status changes when assignment changes.

[[IMAGE:assets/qm3_midterm_v1.3.0/l6_compliance_late_air.png]]
...
**Visual aid:** See the Lecture 6 map above for the compliance and Angrist-Imbens-Rubin (AIR) assumption checklist that turns the Wald ratio into a Local Average Treatment Effect (LATE).
...
Sources: QM3 lecture slides L1-L6 (`QM3_L1_Intro.pdf`, `QM3_L2_CaInf.pdf`, `QM3_L3_Reg.pdf`, `QM3_L4_Reg2.pdf`, `QM3_L5_IV.pdf`, `QM3_L6_IV2.pdf`); `QM3_Midterm_Lecture_Reference_v1.2.1_notes.md`; imagegen lecture-map assets in `Study Guides/assets/qm3_midterm_v1.3.0/`; Edgar learning-thread updates on counterfactuals, ATE, ATT, Difference in Means, ITT, TOT, and LATE; Tyche memory files
Agent: Tyche
---
