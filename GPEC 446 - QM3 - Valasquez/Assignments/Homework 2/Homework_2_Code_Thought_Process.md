...
The code uses `bigimp` to identify the year of a country's largest governance improvement. It estimates a TWFE model for the exact improvement year and then builds an event-study figure using years before and after the event.
...
- Average residual GDP by event time.
- Plot mean residual GDP against years relative to the governance-improvement year.
...
The code uses residual GDP after removing country and year effects because the event-study should focus on deviations from normal country levels and common year patterns, not permanent cross-country differences.
