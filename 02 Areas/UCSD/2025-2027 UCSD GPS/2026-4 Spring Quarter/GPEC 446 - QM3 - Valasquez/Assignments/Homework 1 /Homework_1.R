...
copy_table_disclosure <- paste(
  "---",
  "Generated for: Edgar Agunias",
  paste0("Date: ", Sys.Date()),
  "Model: GPT-5 Codex",
  "Sources: Homework_1.R, atlas.csv, AER::Fertility2, generated Homework 1 HTML table fragments",
  "Agent: Tyche with Hephaestus-style HTML implementation",
  "---",
  sep = "\n"
)

write_copy_table_document <- function(title, body_html, file) {
  html <- paste0(
    "<!doctype html>\n",
    "<html lang=\"en\">\n",
    "<head>\n",
    ...
    "<body>\n",
    body_html,
    "\n<div class=\"disclosure\">", html_escape(copy_table_disclosure), "</div>\n",
    "</body>\n",
    "</html>\n"
  )
  writeLines(html, file)
...
write_copy_table_pages <- function(table_map) {
  table_blocks <- lapply(seq_len(nrow(table_map)), function(i) {
    source_file <- table_map$source[[i]]
    heading <- table_map$heading[[i]]
    output_file <- table_map$output[[i]]

    if (!file.exists(source_file)) {
      stop("Missing source table: ", source_file)
    }

    table_fragment <- paste(readLines(source_file, warn = FALSE), collapse = "\n")
    block <- paste0(
    ...
    body <- paste0(
      "<h1>GPEC 446 Homework 1 Table</h1>\n",
      "<p class=\"copy-note\">Select the table itself in the browser and paste into Word.</p>\n",
      block
    )
    write_copy_table_document(heading, body, output_file)
    block
  })

  combined_body <- paste0(
    "<h1>GPEC 446 Homework 1 Copy-Paste Tables</h1>\n",
    "<p class=\"copy-note\">These tables reuse the computed Homework 1 values from this R script and are formatted for copying into Word.</p>\n",
    paste(table_blocks, collapse = "\n")
  )
  write_copy_table_document(
    "GPEC 446 Homework 1 Copy-Paste Tables",
    combined_body,
    "Homework_1_Tables_Copy_Paste.html"
  )
}
...
################################################################################
# Word Copy-Paste HTML Wrapper Pages
################################################################################

# Regenerate one Word-friendly wrapper per table plus a combined page. These
# wrappers do not recompute results; they collect the table fragments produced
# above and apply simple Times-style CSS that survives copying into Word.
copy_table_map <- tibble::tribble(
  ...
  "table_open_urban_rural_regression.html", "Open Question: Urban/Rural Proxy Regression", "copy_table_open_urban_rural_regression.html",
...
write_copy_table_pages(copy_table_map)
