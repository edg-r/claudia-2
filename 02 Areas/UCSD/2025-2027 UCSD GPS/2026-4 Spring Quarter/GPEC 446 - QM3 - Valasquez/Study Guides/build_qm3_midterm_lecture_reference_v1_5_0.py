...
SOURCE = BASE / "QM3_Midterm_Lecture_Reference_v1.5.0_notes.md"
OUT = BASE / "QM3_Midterm_Lecture_Reference_v1.5.0.pdf"
TITLE = "QM3 Midterm Lecture Reference"
SUBTITLE = "Lecture-slide based reference sheet, v1.5.0"
...
class ConceptDiagram(Flowable):
    ...
    def draw_did_table(self, c):
        self._label(c, self.width / 2, self.height - 18, "DiD: compare changes, not levels", 11, True)
        x0, y0 = 38, 24
        widths = [100, 92, 92, 92]
        rows = [
            ["Group", "Pre", "Post", "Change"],
            ["Treated", "A / 145", "B / 180", "B-A = 35"],
            ["Control", "C / 165", "D / 170", "D-C = 5"],
        ]
        h = 24
        for i, row in enumerate(rows):
            y = y0 + (len(rows) - 1 - i) * h
            for j, cell in enumerate(row):
                x = x0 + sum(widths[:j])
                c.setFillColor(DARK if i == 0 else (colors.HexColor("#E8F3FF") if i == 1 else colors.HexColor("#FFF4E0")))
                c.setStrokeColor(LINE)
                c.rect(x, y, widths[j], h, fill=1, stroke=1)
                self._label(c, x + 6, y + 8, cell, 8.5, i == 0 or j == 0, colors.white if i == 0 else DARK, align="left")
        self._label(c, self.width / 2, 10, "DiD = (B-A) - (D-C) = 30", 12, True, BLUE)

    def draw_twfe(self, c):
        self._label(c, self.width / 2, self.height - 18, "TWFE removes unit baselines and common time shocks", 11, True)
        self._box(c, 32, 72, 112, 34, "Unit FE alpha_i", colors.HexColor("#E8F3FF"))
        self._box(c, 182, 72, 118, 34, "Time FE lambda_t", colors.HexColor("#FFF4E0"))
        self._box(c, 338, 72, 112, 34, "Treatment D_it", colors.HexColor("#E8F8EF"))
        self._arrow(c, 88, 70, 225, 40, colors.HexColor("#7A8A99"))
        self._arrow(c, 241, 70, 255, 40, colors.HexColor("#7A8A99"))
        self._arrow(c, 394, 70, 285, 40, BLUE)
        self._box(c, self.width / 2 - 96, 20, 192, 32, "Y_it after cleanup", colors.white)
        self._label(c, self.width / 2, 8, "Compare each unit to itself over time, net of shared period shocks.", 10, True)

    def draw_event_study(self, c):
        self._label(c, self.width / 2, self.height - 18, "Event study: pre-trends and dynamic treatment effects", 11, True)
        ox, oy = 58, 42
        c.setStrokeColor(GREY)
        c.line(ox, oy, self.width - 42, oy)
        c.line(ox, 22, ox, self.height - 42)
        c.setStrokeColor(colors.HexColor("#C43B3B"))
        c.setDash(3, 2)
        c.line(self.width / 2, 22, self.width / 2, self.height - 42)
        c.setDash()
        pts = [(92, 48), (142, 45), (192, 49), (242, 47), (292, 66), (342, 82), (392, 98)]
        c.setStrokeColor(BLUE)
        c.setLineWidth(1.3)
        ...
        for x, y in pts:
            c.setFillColor(DARK)
            c.circle(x, y, 3, fill=1, stroke=0)
        self._label(c, 160, 28, "pre-period betas near 0", 9, True, GREY)
        self._label(c, 348, 28, "post effects can evolve", 9, True, BLUE)
        self._label(c, self.width / 2 + 4, self.height - 45, "event", 9, True, colors.HexColor("#C43B3B"), align="left")

    def draw_staggered(self, c):
        self._label(c, self.width / 2, self.height - 18, "Staggered adoption: beware already-treated controls", 11, True)
        x0, y0 = 54, 32
        cols = ["t1", "t2", "t3", "t4", "t5", "t6"]
        groups = [("Early", 2), ("Late", 4), ("Never", 99)]
        cell_w, cell_h = 54, 22
        for j, col in enumerate(cols):
            self._label(c, x0 + 82 + j * cell_w + cell_w / 2, y0 + 78, col, 8.5, True)
        for i, (name, start) in enumerate(groups):
            y = y0 + (2 - i) * cell_h
            self._label(c, x0, y + 7, name, 9, True, align="left")
            for j in range(len(cols)):
                x = x0 + 82 + j * cell_w
                treated = j + 1 >= start
                c.setFillColor(colors.HexColor("#E8F8EF") if treated else colors.white)
                c.setStrokeColor(LINE)
                c.rect(x, y, cell_w, cell_h, fill=1, stroke=1)
                if treated:
                    self._label(c, x + cell_w / 2, y + 7, "D=1", 8, True)
        self._label(c, self.width / 2, 10, "Forbidden comparison: late-treated units compared to already-treated early units.", 10, True)

    def draw_falsification(self, c):
        self._label(c, self.width / 2, self.height - 18, "Falsification tests raise or lower confidence; they do not prove PT", 11, True)
        items = [
            ("Prior placebo", "pretend treatment happened early"),
            ("Placebo outcome", "test an outcome treatment should not move"),
            ("Alt control", "use a group treatment cannot reach"),
        ]
        for idx, (head, body) in enumerate(items):
            y = 78 - idx * 32
            self._box(c, 42, y, 122, 24, head, colors.HexColor("#E8F3FF"))
            self._arrow(c, 170, y + 12, 215, y + 12)
            self._label(c, 224, y + 7, body, 9.5, False, DARK, align="left")
...
def main():
    ...
    story = [
        ...
        para(
            "Built from Quantitative Methods 3 (QM3) lecture slides Lecture 1 through Lecture 10 (L1-L10), plus the DiD/FE/FD exercise handouts from the local QM3 inbox. The document is deliberately exam-facing: concept, lecture source, practical use, traps, cues, and formulas.",
            styles["Body"],
        ),
