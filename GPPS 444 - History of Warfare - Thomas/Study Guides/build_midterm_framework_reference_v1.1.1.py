...
OUT = "GPPS_444_Midterm_Framework_Reference_v1.1.1.pdf"
DOC_TITLE = "GPPS 444 Midterm Framework Reference v1.1.1"
ASSET_DIR = "assets/midterm_framework_reference_v1.1.0"
...
PALE_GREEN = colors.HexColor("#EFFAF3")
...
styles.add(
    ParagraphStyle(
        ...
        backColor=colors.HexColor("#FFFDF6"),
...
visual_specs = {
    "thomas_battle_analysis": {
        "graph": "Variables -> decisive pressure -> battle outcome",
        "asset": f"{ASSET_DIR}/01_battle_outcome_framework.png",
        ...
        "eli5": "Battles are not won by one magic thing. A good answer asks which ingredient mattered most in this case, then shows how the other ingredients helped or got in the way.",
    },
    "western_way_war": {
        "graph": "Discipline + finance + adaptation -> repeatable combat power",
        "asset": f"{ASSET_DIR}/02_western_way_of_war.png",
        ...
        "eli5": "The course is tracking a habit: organized states learn how to turn money, drill, and weapons into repeatable battlefield power. The trap is pretending only Europe invented useful ways to fight.",
    },
    "hoplite_phalanx": {
        "graph": "Narrow terrain + shield wall -> small force offsets mass",
        "asset": f"{ASSET_DIR}/03_hoplite_phalanx_terrain.png",
        ...
        "eli5": "A smaller group can beat or delay a bigger one if it chooses ground where the bigger group cannot spread out. The phalanx is strong when everyone stays locked together.",
    },
    "roman_adaptation": {
        "graph": "Defeat -> learning -> engineering -> expansion",
        "asset": f"{ASSET_DIR}/04_roman_adaptation_cycle.png",
        ...
        "eli5": "Rome's superpower was learning. When one method failed, Rome copied, reorganized, built roads and camps, and came back with a system that could keep fighting.",
    },
    "fortress_siege": {
        "graph": "Fortress controls space -> attacker pays in time, labor, food",
        "asset": f"{ASSET_DIR}/05_medieval_siege_defense.png",
        ...
        "eli5": "A castle is like a locked door in the middle of the map. The attacker can break it, starve it, bypass it, or wreck the neighborhood, but every choice costs time and supplies.",
    },
    "chevauchee": {
        "graph": "Fast raid -> economic pain -> political pressure -> bad battle choice",
        "asset": f"{ASSET_DIR}/06_chevauchee_route_map.png",
        ...
        "eli5": "A chevauchee says: if I cannot crack every castle, I can move fast and make you look unable to protect your people. The point is pressure, not just destruction.",
    },
    "infantry_revolution": {
        "graph": "Cheap disciplined infantry + prepared ground -> cavalry shock fails",
        "asset": f"{ASSET_DIR}/07_late_medieval_infantry_tactics.png",
        ...
        "eli5": "The lesson is not 'bows beat knights' by themselves. Bows, stakes, mud, discipline, and a smart position together make the knight's usual charge stop working.",
    },
    "gunpowder_trace": {
        "graph": "Cannon breaks walls -> bastions absorb cannon -> war gets costlier",
        "asset": f"{ASSET_DIR}/08_gunpowder_trace.png",
        ...
        "eli5": "Cannon made old walls weak, so engineers invented better walls. That did not make war simple; it made war expensive, technical, and dependent on states that could pay.",
    },
    "ottoman_standing": {
        "graph": "Standing forces + artillery + frontier logistics -> imperial reach",
        "asset": f"{ASSET_DIR}/09_ottoman_standing.png",
        ...
        "eli5": "The Ottoman case reminds you that military innovation was not only European. Standing troops, artillery, and frontier logistics gave the empire serious staying power.",
    },
    "dynastic_firepower": {
        "graph": "Drill + volley fire + light artillery + cavalry coordination -> combined arms",
        "asset": f"{ASSET_DIR}/10_dynastic_firepower.png",
        ...
        "eli5": "Gunpowder only matters if soldiers can use it together. Breitenfeld is about drill, timing, guns, cavalry, and command all working as one system.",
    },
    "nation_in_arms": {
        "graph": "Nation + conscription + corps mobility -> decisive concentration",
        "asset": f"{ASSET_DIR}/11_nation_in_arms.png",
        ...
        "eli5": "Napoleon could move many motivated soldiers in separate pieces, then snap them together at the key moment. Politics supplied the people; organization made them fast.",
    },
    "napoleonic_limits": {
        "graph": "Distance + scorched earth + weather + coalition learning -> system breaks",
        "asset": f"{ASSET_DIR}/12_napoleonic_limits.png",
        ...
        "eli5": "The same system that looked unbeatable near home became fragile when it had to cross huge distances, find food in empty places, and fight enemies who had learned the trick.",
    },
}
...
def visual_slot(fw, width):
    spec = visual_specs[fw["anchor"]]
    graph = Paragraph(
        f"<b>CONCEPTUAL GRAPH</b><br/>{spec['graph']}<br/><font color='#666666'>Asset: {spec['asset']}</font>",
        styles["VisualBody"],
    )
    image = RLImage(
        spec["asset"],
        width=width * 0.64,
        height=width * 0.64 * 941 / 1672,
    )
    t = Table([[graph, image]], colWidths=[width * 0.36, width * 0.64])
    t.setStyle(
        TableStyle(
            [
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def add_framework(story, fw, width):
    ...
    story.append(visual_slot(fw, width))
    ...
    story.append(para(f"<b>ELI5 CONCLUSION</b> - {visual_specs[fw['anchor']]['eli5']}", "ELI5"))
...
def build():
    ...
    desc = Table(
        [[
            para(
                "Exam-ready midterm reference built from existing Ares study guides, Thomas's course framework, the extracted syllabus, machine-readable slides, and the Parker-edited TCHW text. Version 1.1.1 keeps the framework-page architecture while embedding the completed conceptual PNG assets, compact graph statements, and ELI5 conclusions for fast recall.",
                "Small",
            )
        ]],
        colWidths=[width],
    ...
    story.append(
        para(
            "Generated for: Edgar Agunias<br/>Date: 2026-04-30<br/>Model: GPT-5 Codex<br/>Sources: GPPS 444 extracted syllabus, Ares memory, machine-readable course slides and handouts, existing Study Guides, and machine-readable TCHW PDF text<br/>Agent: Ares",
            "TinyGrey",
        )
    )
