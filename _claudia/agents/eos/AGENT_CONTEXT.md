...
- **2026-06-01 dispatch diagnostics**: `gcalcli` successfully lists all six dispatch calendars, including `006 UCSD Office Hours`, and the parent-added personal block propagated quickly. UCSD Gmail local OAuth reported `invalid_grant` again, so dispatches should keep the re-auth command visible when blocked. Time tracker sync can fail if `admin/GPS_TimeTracker_Spring2026.xlsx - Time Log.csv` is absent; record the missing CSV and continue after regenerating `_claudia/dashboard.html`.
- **2026-06-01 UCSD Gmail re-auth cleared**: Edgar completed the UCSD Gmail re-auth in the parent thread. Follow-up checks of `_claudia/gmail_dispatch_json.py` authenticated as `eagunias@ucsd.edu` and returned 8 unread inbox messages for both the default unread-inbox dispatch query and the requested two-day `to:eagunias@ucsd.edu` query.
